...
public static class EcuDataParser
{
    ...
    public static List<EcuObjectDefinition> ParseObjectList(byte[] payload, IEcuDefinitionService definitionService)
    {
        ...
        ushort entityCount = BitConverter.ToUInt16(payload, 4);
        int offset = 6;
        ...
        for (int i = 0; i < entityCount; i++)
        {
            ...
            ushort id = BitConverter.ToUInt16(payload, offset);
    ...
    public static List<(ushort Id, byte Type)> ParseSetStateResponse(byte[] payload)
    {
        ...
        ushort entityCount = BitConverter.ToUInt16(payload, 1);
        int offset = 3;
        ...
        IEcuDefinitionService definitionService)
    {
        ...
        {
            int size = GetSizeForDataType(type);
            ...
            string name = definitionService.TryGetObject(id, out var def) && def is not null 
                ? def.Name 
                : $"ID_{id}";
    ...
    public static DriverData? ParseDriverData(byte[] payload, IEcuDefinitionService definitionService)
    {
        ...
        string name = definitionService.TryGetObject(id, out var def) && def is not null ? def.Name : $"Driver_{id}";
